...
// Select category container from DOM
...
const allCategories = () => {
  fetch("https://openapi.programming-hero.com/api/categories")
    .then((res) => res.json())
    .then((categories) => {
      const allCat = categories.categories; // Extract categories
      showCategories(allCat); // Show categories in UI
    })
    .catch((err) => {
      console.log(err); // Handle error
    });
};
allCategories(); // Call function

// Show category buttons
const showCategories = (categories) => {
  categories.forEach((categorie) => {
    categoriesContainer.innerHTML += `
    <button id="cat-btn-${categorie.id}" onclick="showCard(${categorie.id})"
     class="btn btn-soft flex justify-start border-none my-1 
     bg-[#e7ffef] w-full hover:bg-[#15803D] cat-btn">${categorie.category_name}</button>
            `;
  });
};
...
const showCard = (id) => {
  manageSpeen(true); // Show loader
  const url = `https://openapi.programming-hero.com/api/category/${id}`;
  fetch(url)
    .then((res) => res.json())
    .then((data) => {
      displayCards(data.plants); // Display cards
...
fetch(url)
  ...
  .then((data) => {
    ...
    displayCards(allData); // Show all plants
  });
...
const displayCards = (datas) => {
  ...
  manageSpeen(false); // Hide loader
};

// Add item to cart
document.getElementById("card-container").addEventListener("click", (e) => {
  if (e.target.innerText === "Add to Cart") {
    const title = e.target.parentNode.children[1].innerText; // Plant name
    const price =
      e.target.parentNode.children[3].children[1].children[1].innerText; // Plant price
...
const deleteCart = (cartId) => {
  ...
  if (filterCarts !== -1) {
    carts.splice(filterCarts, 1); // Remove item
  }

  showCart(carts); // Refresh cart UI
};
...
const displayModal = (plants) => {
  ...
  modalContainer.innerHTML = `
    ...
    `;
  document.getElementById("modalCard").showModal(); // Show modal popup
};
